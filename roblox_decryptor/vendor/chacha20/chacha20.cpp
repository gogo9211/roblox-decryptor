...
{
	const uint8_t* magic_constant = (uint8_t*)"\xCC\xCC\xCC\xCC\xFF\xFF\xFF\xFF\xAA\xAA\xAA\xAA\x55\x55\x55\x55";
	...
	// Transform the key
	uint32_t previous_result = pack4(magic_constant + 3 * 4);

	uint32_t* word_start = (uint32_t*)key;
	uint32_t* current_data = word_start;

	while (current_data < (uint32_t*)(key + sizeof(ctx->key)))
	{
		uint32_t previous_value = word_start[previous_result & 7];
		uint32_t starting_value = *current_data;

		starting_value += previous_value;
		starting_value = rotl32(starting_value, 4);
		starting_value ^= previous_result;

		*current_data = starting_value;
		previous_result = *current_data;
		current_data++;
	}
...
void chacha20_init_context(struct chacha20_context *ctx, uint8_t key[], uint64_t counter)
{
